# Senko Theme (Windows), Preview. <br>
# You should download others from [Hi-doki (Michii)](https://github.com/Hi-doki) instead.

<img width="1920" height="1080" alt="{C06D71D4-0573-41A2-AF34-A64A40EF1DF9}" src="https://github.com/user-attachments/assets/ff4388b1-5103-4ffe-8d13-de2b775dfbff" />


> This repo contains a modified version of a YASB config. <br>
> `Original YASB Theme`: [GitHub Repo](https://github.com/amnweb/yasb-themes/tree/main/themes/764a14e0-2d6a-4826-994a-9289857886ba) <br>

## This repo includes configs for:
- [YASB](#yasb)

## Requirements
- JetbrainsMono Nerd Font

## YASB
<img width="1920" alt="{48329DC2-5D2F-4DFB-BFB2-DB3675E4DE1A}" src="https://github.com/user-attachments/assets/45cada3b-67fa-4302-99a1-1e725a9d1871" />
